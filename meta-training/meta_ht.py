...
class Meta(nn.Module):
    ...
    def __init__(self, args, model):
        ...
        self.linear_gather = self.init_classification_head()
    ...
    def init_classification_head(self):
        """Initialize different linear heads for  different dataset tasks.
        """
        linear_gather = []
        for task_idx in range(self.num_tasks):
            num_classes = data_loader._dataset_class_num(self.args.meta_datasets[task_idx])
            self.model.reset_classifier(num_classes)
            linear_gather.append(deepcopy(self.model.get_classifier()).to(self.args.device))
        ...
        for task_idx in range(task_num):
            ...
            if self.args.wo_da:
                ...
                if self.meta_optim_choose == "reptile":
                    cur_task_optim = optim.SGD([
                        {'params':fast_prompter.parameters(), 'lr':self.update_lr, 'weight_decay':self.weight_decay}, 
                        {'params':self.linear_gather[task_idx].parameters(), 'lr':0.1, 'weight_decay':0}
                    ])
                ...
                fast_prompter_gather, fast_params_gather = [], []
                for i in range(self.num_coarse_classes_meta[task_idx]):
                    fast_prompter_gather.append(deepcopy(self.prompter))
                    fast_params_gather.append(
                        {'params':fast_prompter_gather[i].parameters(), 'lr':self.update_lr, 'weight_decay':self.weight_decay}
                    )
                fast_params_gather.append(
                    {'params':self.linear_gather[task_idx].parameters(), 'lr':0.1, 'weight_decay':0}
                )
                ...
                # the optimizer for each sub-task
                if self.meta_optim_choose == "reptile":
                    cur_task_optim = optim.SGD(fast_params_gather)
            ...
            logits = self.linear_gather[task_idx](output)
            ...
            with torch.no_grad():
                ...
                logits_q = self.linear_gather[task_idx](output_q)
    ...
    def finetuning(self, test_data, prompter=None):
        ...
        if self.args.wo_da:
            ...
            optimizer = torch.optim.SGD([
                {'params':prompter.parameters(), 'lr':1.0, 'momemtum':0.9, 'weight_decay':0.1}, 
                {'params':self.model.get_classifier().parameters(), 'lr':1.0, 'momemtum':0.9, 'weight_decay':0}
            ])
        else:
            ...
            for i in range(self.num_coarse_classes):
                ...
                prompter_params_gather.append(
                    {'params':prompter_gather[i].parameters(), 'lr':1.0, 'momemtum':0.9, 'weight_decay':0.1}
                )
            prompter_params_gather.append(
                {'params':self.model.get_classifier().parameters(), 'lr':1.0, 'momemtum':0.9, 'weight_decay':0}
            )
