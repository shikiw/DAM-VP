...
class Meta(nn.Module):
    ...
    def computer_meta_weight(self, task_diff_weights):
        ...
        for i, each in enumerate(task_diff_weights[0]):
            ...
            # logger.info(keys[i], diff.dtype, dic[keys[i]].dtype)
            dic[keys[i]] -= self.args.meta_step_size * diff
        self.prompter.load_state_dict(dic)


    def forward(self, batch_data, class_names):
        ...
        for task_idx in range(task_num):
            ...
            text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") \
                for c in class_names[self.args.meta_datasets[task_idx]]]).to(self.args.device)
            ...
            prompted_x_spt = self.get_prompted_image(x_spt, prompter=fast_prompter) \
                if self.args.wo_da else self.get_prompted_image(x_spt, text_inputs, self.prototype_gather_meta[task_idx], \
                prompter_gather=fast_prompter_gather)
            logits = self.get_logits(prompted_x_spt, text_inputs)[0]
            ...
            with torch.no_grad():
                ...
                logits_q = self.get_logits(prompted_x_spt_q, text_inputs)[0]
    ...
    def coarse_clustering(self, data_loader, class_names, mode="task_adapting"):
        ...
        if mode == "meta_training":
            ...
            for task_idx in range(task_num):
                # birch = cluster.Birch(n_clusters=None, threshold=22.5, branching_factor=200)
                hc = cluster.AgglomerativeClustering(
                    n_clusters=None, 
                    linkage='average', 
                    distance_threshold=META_TRAIN_DATASETS[self.args.meta_datasets[task_idx]]
                )
                with torch.no_grad():
                    text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") \
                        for c in class_names[self.args.meta_datasets[task_idx]]]).to(self.args.device)
                    for i, sample in enumerate(data_loader[task_idx]):
                        ...
                        rep = self.get_logits(image, text_inputs)[0]
                        if i < 1:
                            rep_gather = rep
                        else:
                        ...
                        if rep_gather.size(0) > 1000:
                            ...
                            break
                # y_pred = birch.fit_predict(rep_gather.detach().cpu().numpy())
    ...
    def get_prompted_image(self, image, text_inputs=None, prototype_gather=None, prompter=None, prompter_gather=None):
        ...
        if self.args.wo_da:
            ...
            with torch.no_grad():
                ...
                rep_batch = self.get_logits(image, text_inputs)[0] # [N, emd_dim]
    ...
    def finetuning(self, test_data, test_class_names, prompter=None):
        ...
        text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") \
            for c in test_class_names]).to(self.args.device)
        BEST_ACC_VAL = -np.inf
        if self.args.wo_da:
        ...
        for epoch in range(20):
            ...
            for i, sample in enumerate(train_loader):
                ...
                logits = self.get_logits(prompted_image, text_inputs)[0]
