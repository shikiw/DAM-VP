...
class TXTDataset(torch.utils.data.Dataset):
    ...
    def get_anno(self):
        anno_path = os.path.join(self.data_dir, "veg200_lists/veg_{}.txt".format(self._split))
...
class Veg200Dataset(TXTDataset):
    """Veg200 dataset."""

    def __init__(self, args, split):
        super(Veg200Dataset, self).__init__(args, split)

    def get_imagedir(self):
        return os.path.join(self.data_dir, "veg200_images/")
...
if __name__ == '__main__':
    ...
    dataset_train = Veg200Dataset(args, "train")
    dataset_val = Veg200Dataset(args, "val")
    dataset_test = Veg200Dataset(args, "test")
